...
控制端在设置中分别启停各协议的局域网发现，修改立即生效并持久化：

| 协议 | 服务类型 | 默认 |
| --- | --- | --- |
| Removent | `_removent._udp.local.` | 开启 |
| VNC / Apple Remote Desktop | `_rfb._tcp.local.` | 关闭 |
| RDP | `_rdp._tcp.local.`、`_ms-wbt-server._tcp.local.` | 关闭 |

关闭浏览不停止被控端广播或共享，也不影响已保存连接及现有会话。兼容服务使用 SRV
地址与端口、实例展示名，不要求或信任 Removent TXT 身份字段；同协议同端点的别名
去重。开启 VNC/RDP 发现时同时扫描直连 IPv4 子网的标准 TCP 端口（5900 / 3389），
校验 RFB 欢迎消息或 X.224/RDP 协商响应后即断开，不登录。每协议并发上限 32、单次超时
1.2 秒；每轮每子网最多 256 个新地址，完成后间隔 30 秒，大网段轮转分批覆盖，并且
每轮重查本机所在 /24 范围及已知端点。排除本机、回环和点对点 VPN 接口。关闭发现同时取消组播查询与端口探测。
广播与扫描结果按协议、地址、端口合并，优先广播名称；未广播的自定义端口需手动添加。
